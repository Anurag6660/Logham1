...
def U1W1(c, thres=None, P=None, flag=1):
    Ue, se, We = scipy.linalg.svd(c, full_matrices= False)
    if flag == 1:
        P = SVR(se, thres)
...
#====================================================
def AUWSP(c, Ndim, d, thres=None, Q=None, flag=1):
    A = {}
    U = {}
    Ures = {}
    W = {}
    s = {}
    P = np.zeros(Ndim-1, dtype=int) if Q is None else Q
    A['%s'%0] = c
    for i in range(Ndim-1):
        U['cx%s'%i], W['%s'%i], s['%s'%i], P[i] = U1W1(A['%s'%i], thres, P[i], flag)
